...
export class MemoryDistiller {
  ...
  async distillEntity(entityID: UUID): Promise<void> {
    const entity = await this.repos.entities.byID(entityID)
    if (!entity) return
    const subjectKind = KIND_TO_SUBJECT[entity.kind] ?? 'topic'
    const events = await this.repos.events.forEntity(entityID, 60)
    if (!events.length) return
    const rels = await this.repos.relationships.forEntity(entityID, 40)
    ...
    await this.repos.memory.upsert({
  ...
  async distillTop(limit = 12): Promise<number> {
    const top = (await this.repos.entities
      .list(undefined, 200))
