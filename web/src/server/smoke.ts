...
async function main(): Promise<void> {
  ...
  const blocks = app.repos.blocks.count()
  const claims = app.repos.claims.count()
  const rows = app.repos.tableRows.count()
  const contradictions = app.repos.contradictions.list()
  const matrix = factMatrix(app.repos)
  const missing = missingProof(app.repos)
  const runs = app.repos.ingestionRuns.list(10)
  const dupRuns = runs.filter((r) => r.status === 'duplicate')
  const audits = app.repos.answerAudits.count()
  ...
  log.app(`SMOKE classification: ${answer.classification ?? '(none)'} | audits=${app.repos.answerAudits.count()}`)
  ...
  const checks: [string, boolean][] = [
    ...
    ['answer audit ledger written', app.repos.answerAudits.count() > audits - 1 && app.repos.answerAudits.count() >= 1]
  ]
