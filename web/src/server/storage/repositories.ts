...
// a shared async libSQL ledger. Only these classes touch SQL; the rest of
...
export class FilesRepo {
  ...
  async upsert(row: Omit<FileRow, 'id'> & { id?: UUID }): Promise<FileRow> {
    const existing = await this.byURL(row.url)
    if (existing) return existing
    const id = row.id ?? newID()
    await this.L.run(
      `INSERT INTO files (id,url,source_type,size_bytes,modified_at,ingested_at,content_hash,availability,alias_of)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [id, row.url, row.sourceType, row.sizeBytes, row.modifiedAt,
  ...
  async byHash(hash: string): Promise<FileRow | null> {
    const r = await this.L.first(
  ...
  async markIngested(id: UUID): Promise<void> {
    await this.L.run(`UPDATE files SET ingested_at=? WHERE id=?`, [nowMs(), id])
  }

  async byURL(url: string): Promise<FileRow | null> {
    const r = await this.L.first(`SELECT * FROM files WHERE url=?`, [url]) as any
    return r ? mapFile(r) : null
  }
  async byID(id: UUID): Promise<FileRow | null> {
    const r = await this.L.first(`SELECT * FROM files WHERE id=?`, [id]) as any
    return r ? mapFile(r) : null
  }
  async list(limit = 500): Promise<FileRow[]> {
    return ((await this.L.all(`SELECT * FROM files ORDER BY ingested_at DESC NULLS LAST, modified_at DESC LIMIT ?`,
      [limit])) as any[]).map(mapFile)
  }
  async remove(id: UUID): Promise<void> {
    await this.L.run(`DELETE FROM files WHERE id=?`, [id])
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM files`)) as any)?.c ?? 0)
  }
  async countByCategory(): Promise<Record<string, number>> {
    const rows = await this.L.all(`SELECT source_type, COUNT(*) c FROM files GROUP BY source_type`) as any[]
...
export class ObjectsRepo {
  ...
  async insert(ko: Partial<KnowledgeObject> & { fileID: UUID; sourceType: SourceType; content: string }): Promise<KnowledgeObject> {
    ...
    await this.L.run(
      `INSERT INTO knowledge_objects (id,file_id,source_type,content,metadata_json,confidence,created_at,updated_at)
       VALUES (?,?,?,?,?,?,?,?)`,
      [id, ko.fileID, ko.sourceType, ko.content, j(ko.metadata ?? {}), ko.confidence ?? 1.0, now, now]
    )
    return {
  ...
  async byID(id: UUID): Promise<KnowledgeObject | null> {
    const r = await this.L.first(
  ...
  async list(limit = 200): Promise<KnowledgeObject[]> {
    return ((await this.L.all(
  ...
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM knowledge_objects`)) as any)?.c ?? 0)
  }
}
...
export class ChunksRepo {
  ...
  async insertMany(chunks: Chunk[]): Promise<void> {
    ...
    const stmts = chunks.map((c) => ({
      sql,
      args: [c.id, c.objectID, c.ordinal, c.text, c.characterRange.lower, c.characterRange.upper,
        c.pageNumber ?? null, c.createdAt, c.contextPrefix ?? null, c.contextPrefixSource ?? null]
    }))
    await this.L.batch(stmts)
  }
  async byObject(objectID: UUID): Promise<Chunk[]> {
    return ((await this.L.all(`SELECT * FROM chunks WHERE object_id=? ORDER BY ordinal`, [objectID])) as any[]).map(mapChunk)
  }
  async byID(id: UUID): Promise<Chunk | null> {
    const r = await this.L.first(`SELECT * FROM chunks WHERE id=?`, [id]) as any
    return r ? mapChunk(r) : null
  }
  async byIDs(ids: UUID[]): Promise<Chunk[]> {
    if (!ids.length) return []
    const q = ids.map(() => '?').join(',')
    return ((await this.L.all(`SELECT * FROM chunks WHERE id IN (${q})`, ids)) as any[]).map(mapChunk)
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM chunks`)) as any)?.c ?? 0)
  }
  ...
  async ftsSearch(query: string, limit = 40, mode: 'and' | 'or' = 'or'): Promise<{ chunkID: UUID; rank: number }[]> {
    const match = toFtsMatch(query, mode)
    if (!match) return []
    try {
      const rows = await this.L.all(
...
export class EntitiesRepo {
  ...
  async upsertCanonical(e: {
    ...
    const found = await this.L.first(`SELECT id FROM entities WHERE kind=? AND normalized=?`, [e.kind, normalized]) as any
    if (found) return found.id
    const id = newID()
    await this.L.run(
      `INSERT INTO entities (id,kind,value,normalized,source_object_id,confidence,attributes_json,quality_tier)
       VALUES (?,?,?,?,?,?,?,?)`,
      [id, e.kind, e.value, normalized, e.sourceObjectID, e.confidence ?? 0.5, '{}', e.qualityTier ?? 'T2']
    )
    return id
  }
  async addMention(entityID: UUID, kind: EntityKind, surface: string, normalized: string, sourceObjectID: UUID, confidence = 0.5): Promise<void> {
    await this.L.run(
      `INSERT INTO entity_mentions (id,entity_id,kind,surface,normalized,source_object_id,confidence)
       VALUES (?,?,?,?,?,?,?)`,
      [newID(), entityID, kind, surface, normalized, sourceObjectID, confidence]
    )
  }
  async byID(id: UUID): Promise<Entity | null> {
    const r = await this.L.first(`SELECT * FROM entities WHERE id=?`, [id]) as any
    return r ? mapEntity(r) : null
  }
  async list(kind?: string, limit = 300): Promise<Entity[]> {
    const rows = kind
      ? await this.L.all(`SELECT e.*, (SELECT COUNT(*) FROM entity_mentions m WHERE m.entity_id=e.id) mc
          FROM entities e WHERE kind=? ORDER BY mc DESC LIMIT ?`, [kind, limit])
      : await this.L.all(`SELECT e.*, (SELECT COUNT(*) FROM entity_mentions m WHERE m.entity_id=e.id) mc
          FROM entities e ORDER BY mc DESC LIMIT ?`, [limit])
    return (rows as any[]).map(mapEntity)
  }
  async search(name: string, limit = 25): Promise<Entity[]> {
    const q = `%${name.toLowerCase()}%`
    return ((await this.L.all(`SELECT * FROM entities WHERE normalized LIKE ? OR lower(value) LIKE ? LIMIT ?`,
      [q, q, limit])) as any[]).map(mapEntity)
  }
  async mentionCount(id: UUID): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM entity_mentions WHERE entity_id=?`, [id])) as any)?.c ?? 0)
  }
  async aliases(id: UUID): Promise<string[]> {
    return ((await this.L.all(`SELECT alias_normalized a FROM entity_aliases WHERE entity_id=?`, [id])) as any[]).map((r) => r.a)
  }
  /** ENTITY_ALIAS_OF — e.g. "Alice Smith" ↔ alice@acme.com from email headers. */
  async addAlias(entityID: UUID, alias: string, source: string): Promise<void> {
    await this.L.run(`INSERT OR IGNORE INTO entity_aliases (entity_id,alias_normalized,source) VALUES (?,?,?)`,
      [entityID, alias.toLowerCase().trim(), source])
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM entities`)) as any)?.c ?? 0)
  }
}
  ...
  async insert(e: {
    ...
    await this.L.run(
      `INSERT INTO events (id,kind,date,end_date,title,summary,source_object_id,confidence,attributes_json,date_confidence,quality_tier,date_precision,narrative_slots_json,slot_values_json,epistemic_status,corroboration_count,status_reason,review_status)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [id, e.kind, e.date, e.endDate ?? null, e.title, e.summary ?? null, e.sourceObjectID,
    ...
    for (const eid of e.entityIDs ?? []) {
      await this.L.run(`INSERT OR IGNORE INTO event_entities (event_id,entity_id) VALUES (?,?)`, [id, eid])
    }
    return mapEvent(await this.L.first(`SELECT * FROM events WHERE id=?`, [id]))
  }
  async byID(id: UUID): Promise<KEvent | null> {
    const r = await this.L.first(`SELECT * FROM events WHERE id=?`, [id]) as any
    return r ? mapEvent(r) : null
  }
  async inRange(start?: number | null, end?: number | null, limit = 400): Promise<KEvent[]> {
    const s = start ?? -8.64e15, e = end ?? 8.64e15
    return ((await this.L.all(`SELECT * FROM events WHERE date>=? AND date<=? ORDER BY date DESC LIMIT ?`,
      [s, e, limit])) as any[]).map(mapEvent)
  }
  async forEntity(entityID: UUID, limit = 200): Promise<KEvent[]> {
    return ((await this.L.all(
      `SELECT e.* FROM events e JOIN event_entities ee ON ee.event_id=e.id WHERE ee.entity_id=? ORDER BY e.date DESC LIMIT ?`,
      [entityID, limit]
    )) as any[]).map(mapEvent)
  }
  async entitiesFor(eventID: UUID): Promise<UUID[]> {
    return ((await this.L.all(`SELECT entity_id FROM event_entities WHERE event_id=?`, [eventID])) as any[]).map((r) => r.entity_id)
  }
  async list(limit = 200): Promise<KEvent[]> {
    return ((await this.L.all(`SELECT * FROM events ORDER BY date DESC LIMIT ?`, [limit])) as any[]).map(mapEvent)
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM events`)) as any)?.c ?? 0)
  }
  async bounds(): Promise<{ earliest: number | null; latest: number | null }> {
    const r = await this.L.first(`SELECT MIN(date) mn, MAX(date) mx FROM events`) as any
    return { earliest: r?.mn != null ? Number(r.mn) : null, latest: r?.mx != null ? Number(r.mx) : null }
  }
  async all(limit = 5000): Promise<KEvent[]> {
    return ((await this.L.all(`SELECT * FROM events ORDER BY date ASC LIMIT ?`, [limit])) as any[]).map(mapEvent)
  }
  async byStatus(status?: EpistemicStatus, limit = 300): Promise<KEvent[]> {
    const rows = status
      ? await this.L.all(`SELECT * FROM events WHERE epistemic_status=? ORDER BY date DESC LIMIT ?`, [status, limit])
      : await this.L.all(`SELECT * FROM events ORDER BY date DESC LIMIT ?`, [limit])
    return (rows as any[]).map(mapEvent)
  }
  async setFactStatus(id: UUID, status: EpistemicStatus, corroboration: number, reason: string): Promise<void> {
    await this.L.run(`UPDATE events SET epistemic_status=?, corroboration_count=?, status_reason=? WHERE id=?`,
      [status, corroboration, reason, id])
  }
  async setReview(id: UUID, review: 'accepted' | 'rejected'): Promise<void> {
    await this.L.run(`UPDATE events SET review_status=? WHERE id=?`, [review, id])
  }
  async statusCounts(): Promise<Record<string, number>> {
    const rows = await this.L.all(`SELECT epistemic_status s, COUNT(*) c FROM events GROUP BY epistemic_status`) as any[]
    const out: Record<string, number> = {}
    for (const r of rows) out[r.s] = Number(r.c)
    return out
  }
  async corroboratedCount(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM events WHERE corroboration_count >= 2`)) as any)?.c ?? 0)
  }
}
...
export class RelationshipsRepo {
  ...
  async upsert(rel: {
    ...
    await this.L.run(
      ...
      [newID(), rel.kind, rel.fromEntityID, rel.toEntityID, rel.viaEventID ?? null,
        rel.sourceObjectID, rel.confidence ?? 0.5, '{}', '[]']
    )
  }
  async forEntity(entityID: UUID, limit = 200): Promise<Relationship[]> {
    return ((await this.L.all(
  ...
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM relationships`)) as any)?.c ?? 0)
  }
}
...
export class MemoryRepo {
  ...
  async upsert(m: {
    ...
    const now = nowMs()
    const existing = await this.bySubject(m.subjectKind, m.subjectIdentifier)
    if (existing) {
      await this.L.run(
        ...
        [m.narrative, j(m.keyEventIDs ?? []), j(m.sourceObjectIDs ?? []),
      ...
      await this.L.run(
        `INSERT INTO memory_objects (id,subject_kind,subject_identifier,key_decisions_json,key_event_ids_json,
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,1,?,?,?)`,
        [newID(), m.subjectKind, m.subjectIdentifier, j(m.keyDecisions ?? []), j(m.keyEventIDs ?? []),
          '[]', j(m.risks ?? []), m.status ?? 'active', m.narrative, j(m.sourceObjectIDs ?? []),
          m.confidence ?? 0.5, now, now, m.qualityTier ?? 'T2']
      )
    }
  }
  async bySubject(kind: SubjectKind, identifier: string): Promise<MemoryObject | null> {
    const r = await this.L.first(`SELECT * FROM memory_objects WHERE subject_kind=? AND subject_identifier=?`,
      [kind, identifier]) as any
    return r ? mapMemory(r) : null
  }
  async list(limit = 200): Promise<MemoryObject[]> {
    return ((await this.L.all(`SELECT * FROM memory_objects ORDER BY updated_at DESC LIMIT ?`, [limit])) as any[]).map(mapMemory)
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM memory_objects`)) as any)?.c ?? 0)
  }
}
...
export class SummariesRepo {
  constructor(private L: Ledger) {}
  async insert(s: { level: Summary['level']; length: Summary['length']; scope: unknown; body: string; modelID?: string | null; confidence?: number }): Promise<void> {
    await this.L.run(
      `INSERT INTO summaries (id,level,length,scope_json,body,produced_at,model_id,confidence) VALUES (?,?,?,?,?,?,?,?)`,
      [newID(), s.level, s.length, j(s.scope), s.body, nowMs(), s.modelID ?? null, s.confidence ?? 0.5]
    )
  }
  async list(level?: string, limit = 100): Promise<Summary[]> {
    const rows = level
      ? await this.L.all(`SELECT * FROM summaries WHERE level=? ORDER BY produced_at DESC LIMIT ?`, [level, limit])
      : await this.L.all(`SELECT * FROM summaries ORDER BY produced_at DESC LIMIT ?`, [limit])
    return (rows as any[]).map(mapSummary)
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM summaries`)) as any)?.c ?? 0)
  }
}
...
export class ConversationsRepo {
  constructor(private L: Ledger) {}
  async create(title?: string): Promise<UUID> {
    const id = newID()
    await this.L.run(`INSERT INTO conversations (id,started_at,title) VALUES (?,?,?)`, [id, nowMs(), title ?? null])
    return id
  }
  async addTurn(conversationID: UUID, ordinal: number, role: 'user' | 'assistant', body: string): Promise<void> {
    await this.L.run(`INSERT INTO conversation_turns (id,conversation_id,ordinal,role,body,created_at) VALUES (?,?,?,?,?,?)`,
      [newID(), conversationID, ordinal, role, body, nowMs()])
  }
  async list(limit = 50): Promise<{ id: UUID; title: string | null; startedAt: number }[]> {
    return ((await this.L.all(`SELECT id,title,started_at FROM conversations ORDER BY started_at DESC LIMIT ?`, [limit])) as any[])
...
export class SavedQueriesRepo {
  constructor(private L: Ledger) {}
  async add(question: string, title?: string, notes?: string, category?: string): Promise<any> {
    const id = newID(); const now = nowMs()
    await this.L.run(`INSERT INTO saved_queries (id,question,title,notes,category,created_at,last_run_at) VALUES (?,?,?,?,?,?,?)`,
      [id, question, title ?? null, notes ?? null, category ?? null, now, null])
    return this.get(id)
  }
  async get(id: UUID): Promise<any> {
    const r = await this.L.first(`SELECT * FROM saved_queries WHERE id=?`, [id]) as any
    return r ? mapSaved(r) : null
  }
  async list(): Promise<any[]> {
    return ((await this.L.all(`SELECT * FROM saved_queries ORDER BY created_at DESC`)) as any[]).map(mapSaved)
  }
  async remove(id: UUID): Promise<void> { await this.L.run(`DELETE FROM saved_queries WHERE id=?`, [id]) }
  async touch(id: UUID): Promise<void> { await this.L.run(`UPDATE saved_queries SET last_run_at=? WHERE id=?`, [nowMs(), id]) }
}
...
export class InvestigationsRepo {
  constructor(private L: Ledger) {}
  async create(question: string): Promise<UUID> {
    const id = newID()
    await this.L.run(`INSERT INTO investigations (id,question,synthesis,created_at,finished_at) VALUES (?,?,?,?,?)`,
      [id, question, null, nowMs(), null])
    return id
  }
  async addStep(investigationID: UUID, ordinal: number, question: string, answerBody: string | null, conf: number | null, citations: UUID[]): Promise<void> {
    await this.L.run(
      `INSERT INTO investigation_steps (id,investigation_id,ordinal,question,answer_body,answer_confidence,answer_citations_json,created_at)
       VALUES (?,?,?,?,?,?,?,?)`,
      [newID(), investigationID, ordinal, question, answerBody, conf, j(citations), nowMs()]
    )
  }
  async finish(id: UUID, synthesis: string): Promise<void> {
    await this.L.run(`UPDATE investigations SET synthesis=?, finished_at=? WHERE id=?`, [synthesis, nowMs(), id])
  }
  async get(id: UUID): Promise<any> {
    const r = await this.L.first(`SELECT * FROM investigations WHERE id=?`, [id]) as any
    if (!r) return null
    const steps = ((await this.L.all(`SELECT * FROM investigation_steps WHERE investigation_id=? ORDER BY ordinal`, [id])) as any[])
  ...
  async list(): Promise<any[]> {
    const rows = await this.L.all(`SELECT id FROM investigations ORDER BY created_at DESC`) as any[]
    return Promise.all(rows.map((r) => this.get(r.id)))
  }
  async remove(id: UUID): Promise<void> { await this.L.run(`DELETE FROM investigations WHERE id=?`, [id]) }
}
...
export class AssertionsRepo {
  constructor(private L: Ledger) {}
  async add(a: Omit<Assertion, 'id' | 'recordedAt' | 'retractedAt'>): Promise<Assertion> {
    const id = newID(); const now = nowMs()
    await this.L.run(
      `INSERT INTO assertions (id,subject_kind,subject_id,predicate,object_kind,object_value,object_entity_id,object_event_id,confidence,evidence_object_ids_json,agent,reason,recorded_at,retracted_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [id, a.subjectKind, a.subjectID, a.predicate, a.objectKind, a.objectValue ?? null,
  ...
  async list(subjectID?: UUID): Promise<Assertion[]> {
    const rows = subjectID
      ? await this.L.all(`SELECT * FROM assertions WHERE subject_id=? AND retracted_at IS NULL ORDER BY recorded_at DESC`, [subjectID])
      : await this.L.all(`SELECT * FROM assertions WHERE retracted_at IS NULL ORDER BY recorded_at DESC LIMIT 300`)
    return (rows as any[]).map(mapAssertion)
  }
  async retract(id: UUID): Promise<void> { await this.L.run(`UPDATE assertions SET retracted_at=? WHERE id=?`, [nowMs(), id]) }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM assertions WHERE retracted_at IS NULL`)) as any)?.c ?? 0)
  }
}
...
export class EventLinksRepo {
  constructor(private L: Ledger) {}
  async insert(link: Omit<EventLink, 'id' | 'createdAt'>): Promise<void> {
    await this.L.run(
      `INSERT INTO event_links (id,source_event_id,target_event_id,relation,confidence,evidence_object_ids_json,allen,source,reason,created_at,superseded_by)
       VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
      [newID(), link.sourceEventID, link.targetEventID, link.relation, link.confidence,
        j(link.evidenceObjectIDs ?? []), link.allen ?? null, link.source, link.reason ?? null, nowMs(), link.supersededBy ?? null]
    )
  }
  async forEvent(eventID: UUID): Promise<EventLink[]> {
    return ((await this.L.all(
...
export class BlocksRepo {
  constructor(private L: Ledger) {}
  async insertMany(blocks: EvidenceBlock[]): Promise<void> {
    const sql =
    ...
    const stmts = blocks.map((b) => ({
      sql,
      args: [b.id, b.objectID, b.ordinal, b.blockType, b.text ?? null, j(b.structuredData ?? {}),
  ...
  async byObject(objectID: UUID): Promise<EvidenceBlock[]> {
    return ((await this.L.all(`SELECT * FROM evidence_blocks WHERE object_id=? ORDER BY ordinal`, [objectID])) as any[]).map(mapBlock)
  }
  async byIDs(ids: UUID[]): Promise<EvidenceBlock[]> {
    if (!ids.length) return []
    const q = ids.map(() => '?').join(',')
    return ((await this.L.all(`SELECT * FROM evidence_blocks WHERE id IN (${q})`, ids)) as any[]).map(mapBlock)
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM evidence_blocks`)) as any)?.c ?? 0)
  }
}
...
export class ClaimsRepo {
  constructor(private L: Ledger) {}
  async insertMany(claims: Omit<LedgerClaim, 'id' | 'createdAt'>[]): Promise<number> {
    const sql =
      `INSERT INTO claims (id,claim_text,claim_type,asserted_by,source_object_id,evidence_block_ids_json,confidence,extraction_method,created_at)
       VALUES (?,?,?,?,?,?,?,?,?)`
    const stmts = claims.map((c) => ({
      sql,
      args: [newID(), c.claimText, c.claimType, c.assertedBy ?? null, c.sourceObjectID,
        j(c.evidenceBlockIDs ?? []), c.confidence, c.extractionMethod, nowMs()]
    }))
    await this.L.batch(stmts)
    return claims.length
  }
  async list(objectID?: UUID, limit = 300): Promise<LedgerClaim[]> {
    const rows = objectID
      ? await this.L.all(`SELECT * FROM claims WHERE source_object_id=? ORDER BY created_at DESC LIMIT ?`, [objectID, limit])
      : await this.L.all(`SELECT * FROM claims ORDER BY created_at DESC LIMIT ?`, [limit])
    return (rows as any[]).map(mapLedgerClaim)
  }
  async byType(type: ClaimType, limit = 500): Promise<LedgerClaim[]> {
    return ((await this.L.all(`SELECT * FROM claims WHERE claim_type=? LIMIT ?`, [type, limit])) as any[]).map(mapLedgerClaim)
  }
  async ftsSearch(query: string, limit = 20): Promise<LedgerClaim[]> {
  ...
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM claims`)) as any)?.c ?? 0)
  }
}
...
export class ContradictionsRepo {
  constructor(private L: Ledger) {}
  async upsert(c: Omit<LedgerContradiction, 'id' | 'detectedAt'>): Promise<void> {
    await this.L.run(
      ...
      [newID(), c.kind, c.aKind, c.aID, c.bKind, c.bID, c.explanation, c.resolutionStatus ?? 'unresolved', nowMs()]
    )
  }
  async list(limit = 200): Promise<LedgerContradiction[]> {
    return ((await this.L.all(`SELECT * FROM contradictions ORDER BY detected_at DESC LIMIT ?`, [limit])) as any[]).map(mapContra)
  }
  async byID(id: UUID): Promise<LedgerContradiction | null> {
    const r = await this.L.first(`SELECT * FROM contradictions WHERE id=?`, [id]) as any
    return r ? mapContra(r) : null
  }
  async forItem(id: UUID): Promise<LedgerContradiction[]> {
    return ((await this.L.all(`SELECT * FROM contradictions WHERE a_id=? OR b_id=?`, [id, id])) as any[]).map(mapContra)
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM contradictions WHERE resolution_status='unresolved'`)) as any)?.c ?? 0)
  }
}
...
export class IngestionRunsRepo {
  constructor(private L: Ledger) {}
  async start(fileID: UUID, parser: string): Promise<UUID> {
    const id = newID()
    await this.L.run(`INSERT INTO ingestion_runs (id,file_id,parser,status,started_at) VALUES (?,?,?,?,?)`,
      [id, fileID, parser, 'parsing', nowMs()])
    return id
  }
  async finish(id: UUID, patch: { status: IngestionRun['status']; blocks?: number; chunks?: number; tableRows?: number; claims?: number; embeddings?: number; warnings?: string[] }): Promise<void> {
    await this.L.run(
      `UPDATE ingestion_runs SET status=?, blocks=?, chunks=?, table_rows=?, claims=?, embeddings=?, warnings_json=?, finished_at=? WHERE id=?`,
      [patch.status, patch.blocks ?? 0, patch.chunks ?? 0, patch.tableRows ?? 0,
        patch.claims ?? 0, patch.embeddings ?? 0, j(patch.warnings ?? []), nowMs(), id]
    )
  }
  async list(limit = 100): Promise<IngestionRun[]> {
    return ((await this.L.all(`SELECT * FROM ingestion_runs ORDER BY started_at DESC LIMIT ?`, [limit])) as any[]).map((r) => ({
...
export class TableRowsRepo {
  constructor(private L: Ledger) {}
  async insertMany(rows: Omit<TableRow, 'id' | 'createdAt'>[]): Promise<number> {
    const sql =
      `INSERT INTO table_rows (id,object_id,sheet,row_num,columns_json,created_at) VALUES (?,?,?,?,?,?)`
    const stmts = rows.map((r) => ({
      sql,
      args: [newID(), r.objectID, r.sheet ?? null, r.rowNum, j(r.columns), nowMs()]
    }))
    await this.L.batch(stmts)
    return rows.length
  }
  async byObject(objectID: UUID, limit = 1000): Promise<TableRow[]> {
    return ((await this.L.all(`SELECT * FROM table_rows WHERE object_id=? ORDER BY row_num LIMIT ?`, [objectID, limit])) as any[])
  ...
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM table_rows`)) as any)?.c ?? 0)
  }
}
...
export class ReviewsRepo {
  constructor(private L: Ledger) {}
  async add(r: { targetKind: string; targetID: UUID; action: string; priorValue?: string | null; newValue?: string | null; reason?: string | null; reviewer?: string }): Promise<void> {
    await this.L.run(
      `INSERT INTO reviews (id,target_kind,target_id,action,prior_value,new_value,reason,reviewer,created_at)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [newID(), r.targetKind, r.targetID, r.action, r.priorValue ?? null, r.newValue ?? null,
        r.reason ?? null, r.reviewer ?? 'user', nowMs()]
    )
  }
  async forTarget(targetID: UUID): Promise<{ action: string; priorValue: string | null; newValue: string | null; createdAt: number }[]> {
    return ((await this.L.all(`SELECT * FROM reviews WHERE target_id=? ORDER BY created_at`, [targetID])) as any[])
      .map((r) => ({ action: r.action, priorValue: r.prior_value ?? null, newValue: r.new_value ?? null, createdAt: Number(r.created_at) }))
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM reviews`)) as any)?.c ?? 0)
  }
}
...
export class AnswerAuditsRepo {
  constructor(private L: Ledger) {}
  async add(a: { question: string; intentKind?: string | null; classification?: string | null; answerBody: string;
           citations: unknown[]; contradictions: number; gaps: string[]; confidence: number;
           source?: string | null; llmPurposes: string[] }): Promise<void> {
    await this.L.run(
      `INSERT INTO answer_audits (id,question,intent_kind,classification,answer_body,citations_json,contradictions,gaps_json,confidence,source,llm_purposes_json,created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      [newID(), a.question, a.intentKind ?? null, a.classification ?? null, a.answerBody,
        j(a.citations), a.contradictions, j(a.gaps), a.confidence, a.source ?? null, j(a.llmPurposes), nowMs()]
    )
  }
  async count(): Promise<number> {
    return Number(((await this.L.first(`SELECT COUNT(*) c FROM answer_audits`)) as any)?.c ?? 0)
  }
}
