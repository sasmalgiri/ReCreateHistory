...
import { FolderPlus, ShieldCheck, Sparkles, X } from 'lucide-react'
...
export default function OnboardingScreen({ onClose }: { onClose: () => void }): JSX.Element {
  async function addFolder(): Promise<void> {
    ...
    if (dir) { await km.ingest.addRoot(dir); onClose() }
  }
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="card relative w-[560px] max-w-[92vw] p-8">
        ...
        <div className="mt-5 space-y-3">
          <Feature icon={<ShieldCheck className="h-4 w-4" />} title="Private by default"
            body="Everything runs on your machine via Ollama. Nothing leaves unless you enable cloud." />
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <Button variant="primary" onClick={addFolder}><FolderPlus className="h-4 w-4" /> Choose a folder</Button>
        </div>
      </div>
    </div>
  )
